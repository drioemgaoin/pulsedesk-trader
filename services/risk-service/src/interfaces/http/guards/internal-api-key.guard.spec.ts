...
describe('Given INTERNAL_RISK_API_KEY is not set', () => {
  describe('when any request arrives', () => {
    it('should allow the request through', () => {
      process.env['INTERNAL_RISK_API_KEY'] = '';
      expect(new InternalApiKeyGuard().canActivate(makeCtx({}))).toBe(true);
    });
  });
});

describe('Given INTERNAL_RISK_API_KEY is configured', () => {
  const OLD_ENV = process.env;

  beforeEach(() => { process.env = { ...OLD_ENV, INTERNAL_RISK_API_KEY: 'secret' }; });
  ...
  describe('when a request arrives with no api key header', () => {
    it('should throw UnauthorizedException', () => {
      expect(() => new InternalApiKeyGuard().canActivate(makeCtx({}))).toThrow(UnauthorizedException);
    });
  });
});
