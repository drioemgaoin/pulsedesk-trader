...
export const OrderTicketPanel = forwardRef<
  OrderTicketPanelHandle,
  OrderTicketPanelProps
>(function OrderTicketPanel({ streamStatus = 'connected', lastPrice = null }, ref) {
  ...
  const [watchedSide, watchedOrderType, watchedSymbol, watchedQtyRaw, watchedLimitRaw] =
    useWatch({
      control,
      name: ["side", "type", "symbol", "quantity", "limitPrice"],
    });
  const side = watchedSide ?? "BUY";
  const orderType = watchedOrderType ?? "MARKET";
  const symbol = watchedSymbol ?? "";
  const qtyRaw = watchedQtyRaw ?? "";
  const limitRaw = watchedLimitRaw ?? "";
  ...
  const onSubmit = (data: FormValues) => {
    ...
    const lp =
      data.type === "LIMIT" && data.limitPrice !== ""
  ...
  const notional =
    qty > 0 && priceForNotional > 0 ? qty * priceForNotional : null;
      ...
      {isStale && (
        ...
        >
          {streamStatus === "reconnecting"
            ? "Stream reconnecting — prices may be stale"
        ...
        >
          ...
          <TextField
            {...register("symbol", {
              ...
              pattern: {
                value: /^[A-Z0-9]+$/,
                message: "Uppercase letters/digits only",
              },
            })}
            label="Symbol"
            ...
            FormHelperTextProps={
              positionLabel && !errors.symbol
                ? {
                    sx: {
                      color:
                        currentPosition && currentPosition.unrealizedPnl >= 0
                          ? "trading.uptick"
                          : "trading.downtick",
                    },
                  }
          ...
          <Box>
            ...
            <Controller
              ...
              render={({ field }) => (
                ...
                >
                  <Tooltip
                    title="Buy or sell immediately at the current market price"
                    placement="top"
                  >
                  ...
                  <Tooltip
                    title="Buy or sell only when the price reaches your specified value or better"
                    placement="top"
                  >
          ...
          <TextField
            {...register("quantity", {
              ...
              validate: (v) =>
                Number.isInteger(parseFloat(v)) || "Whole number",
            })}
            label="Quantity"
            type="number"
            fullWidth
