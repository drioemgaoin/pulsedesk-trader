...
function TickerStrip({ tick, status, showWatchlistToggle, onToggleWatchlist }: TickerStripProps) {
  ...
  const priceColorSx = dir === 'up' ? 'trading.uptick' : dir === 'down' ? 'trading.downtick' : 'text.primary';
    ...
    >
      ...
      {showWatchlistToggle && (
        <Tooltip title="Toggle watchlist">
          <IconButton size="small" onClick={onToggleWatchlist} aria-label="toggle watchlist" sx={{ mr: 1 }}>
      ...
      {tick ? (
        <>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 3, flexShrink: 0 }}>
            <Tooltip title="Highest price a buyer is currently willing to pay" placement="bottom">
              <Box sx={{ cursor: 'default' }}>
                <Typography display="block" lineHeight={1} sx={{ fontSize: '0.625rem', letterSpacing: '0.06em', textTransform: 'uppercase', color: 'text.disabled', mb: 0.5 }}>Bid</Typography>
                <Typography variant="body2" sx={{ fontVariantNumeric: 'tabular-nums', fontWeight: 600, lineHeight: 1 }}>
                  {tick.bid.toFixed(2)}
                </Typography>
              </Box>
            </Tooltip>
            <Tooltip title="Lowest price a seller is currently willing to accept" placement="bottom">
              <Box sx={{ cursor: 'default' }}>
                <Typography display="block" lineHeight={1} sx={{ fontSize: '0.625rem', letterSpacing: '0.06em', textTransform: 'uppercase', color: 'text.disabled', mb: 0.5 }}>Ask</Typography>
                <Typography variant="body2" sx={{ fontVariantNumeric: 'tabular-nums', fontWeight: 600, lineHeight: 1 }}>
                  {tick.ask.toFixed(2)}
                </Typography>
              </Box>
          ...
          <Box sx={{ display: { xs: 'none', md: 'flex' }, alignItems: 'center', gap: 3, flexShrink: 0, ml: 3 }}>
            <Divider orientation="vertical" flexItem />
            <Tooltip title="Cost to trade — difference between ask and bid prices. Lower is better." placement="bottom">
              <Box sx={{ cursor: 'default' }}>
                <Typography display="block" lineHeight={1} sx={{ fontSize: '0.625rem', letterSpacing: '0.06em', textTransform: 'uppercase', color: 'text.disabled', mb: 0.5 }}>Spread</Typography>
                <Typography variant="body2" sx={{ fontVariantNumeric: 'tabular-nums', color: 'text.secondary', lineHeight: 1 }}>
                  {spread}
                </Typography>
              </Box>
            </Tooltip>
            <Tooltip title="Total number of shares traded today" placement="bottom">
              <Box sx={{ cursor: 'default' }}>
                <Typography display="block" lineHeight={1} sx={{ fontSize: '0.625rem', letterSpacing: '0.06em', textTransform: 'uppercase', color: 'text.disabled', mb: 0.5 }}>Volume</Typography>
                <Typography variant="body2" sx={{ fontVariantNumeric: 'tabular-nums', color: 'text.secondary', lineHeight: 1 }}>
                  {fmtVol(tick.volume)}
                </Typography>
              </Box>
...
export default function TradingTerminalPage() {
  ...
  useEffect(() => {
    const mapped =
      ...
      status === 'reconnecting' ? 'disconnected' : 'connecting';
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
        ...
        >
          ...
          <Box>
            ...
            <Typography variant="body2" fontWeight={600}>
              {currentFill
                ? `${currentFill.symbol} × ${currentFill.filledQuantity} ${currentFill.side} @ $${currentFill.fillPrice}`
                : ''}
            </Typography>
          </Box>
        </Paper>
      ...
      <Box sx={{ display: 'flex', flex: 1, minHeight: 0 }}>
        ...
        {isLg ? (
          <Box sx={{ width: 220, borderRight: 1, borderColor: 'divider', overflow: 'hidden', flexShrink: 0 }}>
            {watchlistPanel}
          </Box>
        ) : (
          <Drawer
        ...
        <Box sx={{ flex: 1, overflow: 'hidden', borderRight: 1, borderColor: 'divider', minWidth: 0 }}>
          <ChartPanel symbol={selectedSymbol || null} tick={throttledTick} streamStatus={status} />
        </Box>

        {/* Order Ticket — 320px fixed right panel */}
        <Box sx={{ width: 320, overflow: 'hidden', flexShrink: 0, borderLeft: 1, borderColor: 'divider' }}>
          <OrderTicketPanel
            ref={orderTicketRef}
            streamStatus={status}
      ...
      <Box sx={{ borderTop: 1, borderColor: 'divider', flexShrink: 0, overflow: 'hidden', height: bottomOpen ? BOTTOM_PANEL_HEIGHT + 37 : 37 }}>
        <BlotterPanel
          accountId={accountId}
